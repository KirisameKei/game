		...
		for player_cards in match[:len(match)-1]:
			...
			for i in range(dansuu):
				...
				for card in player_cards:
					...
					retsusuu += 1
			uno.save(f"uno_player{player_number}.png")
			player_number += 1
...
async def match_uno(client3, message, about_uno):
	...
	if bahuda[1] == "D":
		...
		for p in player_list:
			if p == higaisya:
				f = discord.File("./uno_player0.png")
				await p.send(
					content=f"{higaisya.name}さんに2ダメージ！！残り{higaisya_remaining}",
					file=f
				)
	...
	while True:
		...
		if isinstance(match[player_number][0], str):
			...
			while True:
				...
				try:
					...
					if not no:
											...
											try:
												...
												for p in player_list:
													if p == player:
														...
														f = discord.File(f"./uno_player{higaisya_number}.png")
														await p.send(
															content=f"{player.name}さんは山札から{hiita_card}を出し{color}に設定しました。残り{remaining}枚\n{higaisya.name}さんに4ダメージ！！残り{higaisya_remaining}",
															file=f
														)
						...
						await player.send("もう一度入力してください")
